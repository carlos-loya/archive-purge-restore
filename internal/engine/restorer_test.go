...
func TestRestoreDryRun(t *testing.T) {
	...
	// Create mock DB to verify no rows are inserted.
	db := &mockDB{
		schema: columns,
		pkCols: []string{"id"},
	}
	...
	// Verify no rows were actually inserted into the DB.
	if len(db.restored) != 0 {
		t.Errorf("expected 0 restored rows in dry-run, got %d", len(db.restored))
	}
}

func TestRestoreDryRunMultipleFiles(t *testing.T) {
	dir := t.TempDir()
	store, err := filesystem.New(dir)
	if err != nil {
		t.Fatal(err)
	...
	rows1 := []database.Row{{"id": int32(1), "val": "a"}}
	rows2 := []database.Row{{"id": int32(2), "val": "b"}, {"id": int32(3), "val": "c"}}
	...
	if result.Tables[0].RowsRestored != 3 {
		t.Errorf("RowsRestored = %d, want 3", result.Tables[0].RowsRestored)
	}
	if len(result.Tables[0].Files) != 2 {
		t.Errorf("len(Files) = %d, want 2", len(result.Tables[0].Files))
	}
...
func TestRestoreDryRunNilDB(t *testing.T) {
	dir := t.TempDir()
	store, err := filesystem.New(dir)
	if err != nil {
		t.Fatal(err)
	...
	// Pass nil for db since dry-run should not need a database connection.
	result, err := restorer.Restore(ctx, opts, nil)
	if err != nil {
		t.Fatalf("Restore() dry-run with nil db error: %v", err)
	}

	if result.Tables[0].RowsRestored != 1 {
		t.Errorf("RowsRestored = %d, want 1", result.Tables[0].RowsRestored)
	}
}
